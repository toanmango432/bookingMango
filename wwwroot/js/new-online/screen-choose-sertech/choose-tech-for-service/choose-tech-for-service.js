...
export async function ChooseTechForServices() {
  ...
  // Kiểm tra cho chọn multi tech không và có đang chọn nhiều service không trước khi hiển thị chọn multitech
  const $wrapChooseMultitech = $(".wrap-choose-multitech");
  ...
  if (isChooseMultiSer && isBookMultipleTech) {
    ...
    `;
    $wrapChooseMultitech.append(htmlChooseMulti);
  } else {
    $wrapChooseMultitech.empty();
  }
...
$(document).ready(async function () {
  ...
  $(document).on("click", ".choose-multitech", async function () {
    ...
    if (!isChooseMultiSer || !isBookMultipleTech) {
      console.warn(
        "Setting is not allow choose multitech or you choosing only one service!"
      );
      return;
    }
    if ($this.hasClass("not-ser")) {
